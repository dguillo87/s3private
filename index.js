...
class AwsClient {
    ...
    static async sign(input, init) {
        ...
        if (typeof input === 'string') {
            ...
            return new Response('Runtime error - can only sign URLs or Requests', {
                status: 400
            })
        }
        if (['us-east', 'us-east-1'].includes(AwsClient.env.S3_REGION)) {
            input.hostname = `s3.amazonaws.com`
        } else {
            input.hostname = `s3-${AwsClient.env.S3_REGION}.amazonaws.com`
        }
        input.pathname = `/${AwsClient.env.S3_BUCKET_NAME}${input.pathname}`

        const signer = new AwsV4Signer(Object.assign({
            url: input
        }, init, AwsClient, init && init.aws))
...
class AwsV4Signer {
    constructor({
        ...
        this.credentialString = [this.datetime.slice(0, 8), AwsClient.env.S3_REGION, 's3', 'aws4_request'].join('/')

        if (this.signQuery) {
            if (!params.has('X-Amz-Expires')) params.set('X-Amz-Expires', 86400)
            params.set('X-Amz-Algorithm', 'AWS4-HMAC-SHA256')
            params.set('X-Amz-Credential', AwsClient.env.ACCESS_KEY_ID + '/' + this.credentialString)
    ...
    async authHeader() {
        return [
            'AWS4-HMAC-SHA256 Credential=' + AwsClient.env.ACCESS_KEY_ID + '/' + this.credentialString,
    ...
    async signature() {
        ...
        const cacheKey = [AwsClient.env.SECRET_ACCESS_KEY, date, AwsClient.env.S3_REGION, 's3'].join()
        let kCredentials = this.cache.get(cacheKey)
        if (!kCredentials) {
            const kDate = await Algo.hmac('AWS4' + AwsClient.env.SECRET_ACCESS_KEY, date)
            const kRegion = await Algo.hmac(kDate, AwsClient.env.S3_REGION)
...
export default {
  ...
  //Workers calls AwsClient.sign with the Worker URL$
  //AwsClient sign transform the Worker URL into a S3 URL and calls AWSv4SIgner
  //AWSv4SIgner signs the URL using the Algo function
  //Workers sends the signed URL to get the S3 content
